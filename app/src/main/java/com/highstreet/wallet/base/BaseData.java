...
//        }
        ...
        while (iterator.hasNext()) {
            Account account = iterator.next();
            if (!(AccountManager.Companion.instance().getChain().getChain().equals(account.baseChain))) {
                iterator.remove();
            }
        }
    ...
    public Account onSelectAccount(String id) {
        ...
        Cursor cursor = getBaseDB().query(BaseConstant.DB_TABLE_ACCOUNT, new String[]{"id", "uuid", "nickName", "isFavo", "address", "baseChain",
                ...
                "isValidator", "sequenceNumber", "accountNumber", "fetchTime", "msize",
                "importTime", "lastTotal", "sortOrder", "pushAlarm", "newBip"}, "id == ?", new String[]{id}, null, null, null);
        if (cursor != null && cursor.moveToFirst()) {
            result = new Account(
                    cursor.getLong(0),
                    cursor.getString(1),
...
//        }
    ...
    public Account onSelectExistAccount(String address, BaseChain chain) {
        ...
        Cursor cursor = getBaseDB().query(BaseConstant.DB_TABLE_ACCOUNT, new String[]{"id", "uuid",
                ...
                "fromMnemonic", "path", "isValidator", "sequenceNumber", "accountNumber",
                "fetchTime", "msize", "importTime", "lastTotal", "sortOrder", "pushAlarm",
                "newBip"}, "address == ?", new String[]{address}, null, null, null);
        if (cursor != null && cursor.moveToFirst()) {
            result = new Account(
                    cursor.getLong(0),
                    cursor.getString(1),
    ...
    public Account onSelectExistAccount2(String address) {
        ...
        Cursor cursor = getBaseDB().query(BaseConstant.DB_TABLE_ACCOUNT, new String[]{"id", "uuid", "nickName", "isFavo", "address", "baseChain",
                "hasPrivateKey", "resource", "spec", "fromMnemonic", "path",
                "isValidator", "sequenceNumber", "accountNumber", "fetchTime", "msize",
                "importTime", "lastTotal", "sortOrder", "pushAlarm", "newBip"}, "address == ?", new String[]{address}, null, null, null);
        if (cursor != null && cursor.moveToFirst()) {
            result = new Account(
                    cursor.getLong(0),
                    cursor.getString(1),
     ...
     * 这个方法也用改变是否备份
     */
    public void onUpdateBackup(Long id) {
        ContentValues values = new ContentValues();
        values.put("pushAlarm", true);
        getBaseDB().update(BaseConstant.DB_TABLE_ACCOUNT, values, "id = ?", new String[]{"" + id});
    }
